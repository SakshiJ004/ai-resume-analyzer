            ...
            <div className="max-w-4xl mx-auto p-6">
                ...
                {resumes.length === 0 ? (
                    <div className="text-center py-20">
                        ...
                        <p className="text-xl font-semibold text-gray-300">No resumes yet</p>
                        <p className="text-gray-500 text-sm mt-2">Upload your first resume to get started</p>
                        <Link to="/analyze" className="inline-block mt-6 bg-indigo-600 hover:bg-indigo-700 px-6 py-2.5 rounded-lg font-semibold transition">
                            Upload Resume
                        </Link>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {/* Version comparison banner — show only if 2+ resumes */}
                        {resumes.length >= 2 && (
                            <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-3 text-sm">
                                    ...
                                    <span className={`text-lg font-bold ${getScoreColor(resumes[0].latestScore)}`}>
                                        {resumes[0].latestScore || '—'}
                                    </span>
                                </div>
                            </div>
                        )}
                            ...
                            <div key={resume._id} className="bg-gray-900 border border-gray-800 hover:border-gray-600 rounded-xl p-5 transition">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 bg-indigo-500/15 rounded-lg flex items-center justify-center text-xl flex-shrink-0">
                                            📄
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                {index === 0 && (
                                                    <span className="bg-indigo-500/15 text-indigo-300 text-xs px-2 py-0.5 rounded-full">
                                                        Latest
                                                    </span>
                                                )}
                                            </div>
                                            <div className="flex items-center gap-4 mt-1">
                                                ...
                                                {resume.parsedData?.skills?.length > 0 && (
                                                    <span className="text-xs text-gray-500">
                                                        {resume.parsedData.skills.length} skills detected
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-3 flex-shrink-0">
                                        <Link
                                            to={`/analyze?resumeId=${resume._id}`}
                                ...
                                {resume.parsedData?.skills?.length > 0 && (
                                    <div className="mt-4 flex flex-wrap gap-1.5">
                                        {resume.parsedData.skills.slice(0, 8).map((skill) => (
                                            <span key={skill} className="bg-gray-800 text-gray-400 text-xs px-2.5 py-1 rounded-full">
                                                {skill}
                                            </span>
                                        ))}
                                        {resume.parsedData.skills.length > 8 && (
                                            <span className="bg-gray-800 text-gray-500 text-xs px-2.5 py-1 rounded-full">
